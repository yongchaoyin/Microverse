extends Control
class_name TerminalChatMinimal
...
func _ready() -> void:
	...
	_append_system("最小化测试场景已准备就绪")
	_append_system("输入内容并按Enter测试输入功能")
	
	print("[TerminalChatMinimal] 测试场景已准备就绪，焦点已设置到输入框")

func _on_input_gui_input(event: InputEvent) -> void:
	# 打印详细的输入事件信息
	print("[TerminalChatMinimal] 输入事件: ", event)
	if event is InputEventKey:
		print("[TerminalChatMinimal] 键盘事件 - 按键码: ", event.keycode, ", 按下状态: ", event.pressed, ", Ctrl: ", event.ctrl_pressed, ", Shift: ", event.shift_pressed)
	
	# 处理 Enter 发送
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_ENTER or event.keycode == KEY_KP_ENTER:
			print("[TerminalChatMinimal] 检测到Enter键")
			_on_send()
			get_viewport().set_input_as_handled()

func _on_send() -> void:
	var t := input.text.strip_edges()
	print("[TerminalChatMinimal] _on_send 被调用，输入内容: '%s'" % t)
	if t == "":
		print("[TerminalChatMinimal] 输入为空，忽略")
		return
	input.text = ""
...
func _append_line(role: String, name: String, text: String, color: Color) -> void:
	print("[TerminalChatMinimal] _append_line 被调用 - role: %s, name: %s, text: %s" % [role, name, text])
	...
	print("[TerminalChatMinimal] 准备添加到output: %s" % final_text)
	output.append_text(final_text)
	output.newline()
	_scroll_to_bottom()
	print("[TerminalChatMinimal] _append_line 完成")
