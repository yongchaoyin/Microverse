...
func _ready() -> void:
	...
	# 模拟一些输入事件进行测试
	_append_system("修复版测试场景已准备就绪")
	_append_system("将模拟输入事件进行测试")
	
	print("[TerminalChatFixed] 测试场景已准备就绪，焦点已设置到输入框")
...
func _simulate_input_events():
	print("[TerminalChatFixed] 开始模拟输入事件")
	
	# 模拟输入文本
	input.text = "测试输入内容"
	print("[TerminalChatFixed] 已设置输入文本: ", input.text)
	
	# 模拟Enter键按下事件
	var enter_event = InputEventKey.new()
	enter_event.keycode = KEY_ENTER
	enter_event.pressed = true
	print("[TerminalChatFixed] 模拟Enter键按下事件")
	...
	# 再测试一次空输入
	input.text = ""
	print("[TerminalChatFixed] 测试空输入")
	_on_send()

func _on_input_gui_input(event: InputEvent) -> void:
	# 打印详细的输入事件信息
	print("[TerminalChatFixed] 输入事件: ", event)
	if event is InputEventKey:
		print("[TerminalChatFixed] 键盘事件 - 按键码: ", event.keycode, ", 按下状态: ", event.pressed, ", Ctrl: ", event.ctrl_pressed, ", Shift: ", event.shift_pressed)
	
	# 处理 Enter 发送
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_ENTER or event.keycode == KEY_KP_ENTER:
			print("[TerminalChatFixed] 检测到Enter键")
			_on_send()
			get_viewport().set_input_as_handled()

func _on_send() -> void:
	var t := input.text.strip_edges()
	print("[TerminalChatFixed] _on_send 被调用，输入内容: '%s'" % t)
	if t == "":
		print("[TerminalChatFixed] 输入为空，忽略")
		return
	input.text = ""
...
func _append_line(role: String, name: String, text: String, color: Color) -> void:
	print("[TerminalChatFixed] _append_line 被调用 - role: %s, name: %s, text: %s" % [role, name, text])
	...
	print("[TerminalChatFixed] 准备添加到output: %s" % final_text)
	output.append_text(final_text)
	output.newline()
	_scroll_to_bottom()
	print("[TerminalChatFixed] _append_line 完成")
